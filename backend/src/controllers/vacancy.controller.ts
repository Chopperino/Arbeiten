...
export class VacancyController {
  ...
  public async getAllComplaints(req: Request<{ vacancyId: string }>, res: Response<ComplaintResponse[]>) {
    const complaints = await this.vacancyService.getAllComplaints(req.params.vacancyId);
    res.status(200).send(complaintsMapper(complaints));
  }
}